...
# ---------------------- REGISTER PAGE ---------------------- #
    ...
    with st.form("register_form"):
        ...
        if submitted:
            ...
            # Store in session to show after rerun
            st.session_state['recent_acc_no'] = acc.account_number
            st.session_state.page = "account_created"
            st.rerun()
elif st.session_state.page == "account_created":
    acc_no = st.session_state.get("recent_acc_no")
    if acc_no:
        st.success("🎉 Account created successfully!")
        st.info(f"🆔 **Your Account Number is: `{acc_no}`**")
        st.write("📌 Please save this number safely. You'll need it to log in.")
        if st.button("🔐 Proceed to Login"):
            st.session_state.page = "login"
            st.rerun()
    else:
        st.warning("⚠️ No account info found. Please register again.")
        st.session_state.page = "register"
        st.rerun()
